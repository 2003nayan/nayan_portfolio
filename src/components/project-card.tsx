      ...
      >
        ...
        {image ? (
          <Image
            ...
            width={500}
            height={300}
            className="h-40 w-full overflow-hidden object-cover object-top border-b border-black/20"
          />
        ): (
          <Image
          ...
          <div className="flex flex-row flex-wrap items-start gap-1">
            {links?.map((link, idx) => (
              <Link href={link?.href || ""} key={idx} target="_blank">
                <Badge key={idx} className={`flex gap-2 px-2 py-1 text-[10px]`} variant={"outline"}>
                  {link.icon}
                  {link.type}
                </Badge>
              </Link>
